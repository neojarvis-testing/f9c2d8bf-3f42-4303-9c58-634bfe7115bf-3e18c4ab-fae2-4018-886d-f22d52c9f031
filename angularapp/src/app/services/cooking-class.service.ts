...
export class CookingClassService {
  ...
  getAllCookingClassRequests(): Observable<CookingClassRequest[]> {
    return this.http.get<CookingClassRequest[]>(`${this.apiUrl}/api/cooking-class-request`, { headers: this.getHeaders() });
  }
  getCookingClassRequestsByUserId(userId: string): Observable<CookingClassRequest[]> {
    return this.http.get<CookingClassRequest[]>(`${this.apiUrl}/api/cooking-class-request/user/${userId}`, { headers: this.getHeaders() });
  }
  addCookingClassRequest(request: CookingClassRequest): Observable<CookingClassRequest> {
    return this.http.post<CookingClassRequest>(`${this.apiUrl}/api/cooking-class-request`, request, { headers: this.getHeaders() });
  }
  updateCookingClassRequest(requestId: number, request: CookingClassRequest): Observable<CookingClassRequest> {
    return this.http.put<CookingClassRequest>(`${this.apiUrl}/api/cooking-class-request/${requestId}`, request, { headers: this.getHeaders() });
  }
  deleteCookingClassRequest(requestId: number): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/api/cooking-class-request/${requestId}`, { headers: this.getHeaders() });
  }
}